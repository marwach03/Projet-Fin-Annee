...
class Mindfulness extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      soundList: [
        { name: 'Cheminé', file: require('../../assets/sound1.wav') },
        { name: 'Pluie et orages', file: require('../../assets/sound2.wav') },
  ...
  async componentDidMount() {
    const { sound } = await Audio.Sound.createAsync(
      require('../../assets/Mindfulness.wav')
    );
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        <Video
          source={require('../../assets/back5.mp4')}
          rate={1.0}
          volume={1.0}
        ...
        <Modal
          ...
          <View style={styles.modalContainer}>
            <View style={styles.modalContent}>
              <Text style={styles.modalTitle}>Choose Sound</Text>
              <View style={styles.soundListContainer}>
                {soundList.map((sound, index) => (
                  <TouchableOpacity
                    key={index}
                    onPress={() => {
                      this.handleSoundSelect(index);
                      this.setState({ modalVisible: false });
                    }}
                    style={[
                      styles.soundListItem,
                      index === selectedSoundIndex && styles.selectedSoundListItem
                    ]}>
                    <Text style={styles.soundListItemText}>{sound.name}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
